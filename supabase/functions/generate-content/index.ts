    ...
    if (save) {
      ...
      if (tableMap[step]) {
        ...
        if (step === 'ofv' && parsedContent) {
          ...
          if (parsedContent.big_promise)
            insertData.big_promise = parsedContent.big_promise;
          if (parsedContent.vehicle_name)
            insertData.vehicle_name = parsedContent.vehicle_name;
          if (parsedContent.vehicle_description)
            insertData.vehicle_description = parsedContent.vehicle_description;
          if (parsedContent.quick_win)
            insertData.quick_win = parsedContent.quick_win;
          if (parsedContent.decision_frame)
            insertData.decision_frame = parsedContent.decision_frame;
          if (parsedContent.guarantee)
            insertData.guarantee = parsedContent.guarantee;
          if (parsedContent.urgency) insertData.urgency = parsedContent.urgency;
          if (parsedContent.social_proof)
            insertData.social_proof = parsedContent.social_proof;
          if (parsedContent.deliverables)
            insertData.deliverables = parsedContent.deliverables;
        }
      ...
      } else if (outputSteps.includes(step) && step !== 'gbp_description') {
        // F-089 R-03 (PARIDAD con generate-content/route.ts, seam
        // `shouldPersistGeneratedOutput`) — el home de `gbp_description` es
        // `gbp_profiles` (lo escribe `generate-gbp`); NO se duplica esa fila en
        // `generated_outputs`. El edge no importa de src/lib (Deno/esm.sh), por eso
        // el guard va inline. Se respeta la divergencia de idioma existente sin ampliarla.
